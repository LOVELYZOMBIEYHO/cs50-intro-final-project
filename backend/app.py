...
def pdf_template(id):
    ...
    # ########################### Make PDF and output
    # kitoptions - this variable allow you to use "url_for" or other local file in the html template ***important
    # css = ['static/pdfStyle.css']
    ...
    # # ########### Make PDF but not output, only use for preview or Flask as frontend 
    # # css = ['static/pdfStyle.css']
    # css = ['templates/bootstrap.min.css']
    # # css = ['templates/tailwind.min.css']
    # kitoptions = {"enable-local-file-access": None}
    # config = pdfkit.configuration(wkhtmltopdf='C:/Program Files/wkhtmltopdf/bin/wkhtmltopdf.exe')
    # pdf = pdfkit.from_string(rendered,False, configuration=config, css=css, options=kitoptions)
